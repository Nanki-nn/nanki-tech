...
第一阶段的 8 篇文章是一张系列地图，不是一份要求一次性生产 8 篇正文的执行计划。设计确认后的下一份执行计划只覆盖目录初始化、系列 `README.md` 和第 01 篇的研究、调试、图表与母稿。第 01 篇经作者确认后，再为第 02 篇制定单独计划；后续文章依次类推。
...
- `未开始`：只有系列地图中的选题；
- `研究中`：正在收集源码和运行时证据；
- `待审阅`：母稿及配套资产已完成，等待作者确认；
- `已确认`：作者已经确认，可以开始规划下一篇。

正式发布状态不在“未发表”目录内维护；文章迁入站点内容目录后由博客清单和合集配置表示发布状态。
...
不适用的字段写明“不适用”，避免空白被误解为遗漏。作者推断不能直接作为正文中的确定结论。
...
下一份执行计划必须止于这个首个里程碑，不得包含第 02～08 篇正文的生产任务。第 02～08 篇只作为 `README.md` 中的系列路线图存在，直到前一篇达到 `已确认` 状态。
